...
def consistency_ensemble(gt_infos_a, gt_infos_b, memory_ensemble_cfg):
    ...
    if not memory_ensemble_cfg.get('CLASS_AGNOSTIC', True):
        print("Doing class based filtering")

        cl_label_a = gt_box_a[:, 7].long()
        cl_label_b = gt_box_b[:, 7].long()
        class_matches = (cl_label_a.reshape(-1, 1) ==  cl_label_b.reshape(1, -1)).float().cpu()
        class_matches[cl_label_a == -1] = 0
        class_matches[:, cl_label_b == -1] = 0

        # if commu_utils.get_rank() == 0:
        #     print("Do class matching for memory ensembling")
        #     # construct a class matches matrix, NxM
        #     print(gt_box_a[:, 7])
        #     print(gt_box_b[:, 7])
        #     print(class_matches) 
    else:
        # print("No class based filtering")
        class_matches = torch.ones_like(iou_matrix)

    # If the class label does not match, then set the IOU to zero
    iou_matrix = iou_matrix * class_matches
