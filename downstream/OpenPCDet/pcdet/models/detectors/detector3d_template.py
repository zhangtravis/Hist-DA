...
from .. import backbones_2d, backbones_3d, dense_heads, roi_heads, history_query
...
class Detector3DTemplate(nn.Module):
    ...
    def set_per_cls_weights(self, per_cls_weights):
        for cur_module in self.module_list:
            cur_module.per_cls_weights = per_cls_weights
    ...
    def build_history_query(self, model_info_dict):
        if self.model_cfg.get("HISTORY_QUERY", None) is None:
            return None, model_info_dict
        history_query_module = history_query.__all__[self.model_cfg.HISTORY_QUERY.NAME](
            model_cfg=self.model_cfg.HISTORY_QUERY,
            num_class=self.num_class
            if not self.model_cfg.HISTORY_QUERY.get("CLASS_AGNOSTIC", True)
            else 1,
            num_point_features=model_info_dict["num_point_features"],
        )
        model_info_dict["num_point_features"] = history_query_module.point_dim
        model_info_dict["module_list"].append(history_query_module)
        return history_query_module, model_info_dict
    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                if "roi_cls_preds" in batch_dict:
                    roi_cls_preds = batch_dict["roi_cls_preds"][batch_mask]
                    record_dict["roi_cls_preds"] = roi_cls_preds[selected]
