...
class DataAugmentor(object):
    ...
    def random_object_rotation(self, data_dict=None, config=None):
        ...
        if data_dict.get('history_scans', None) is not None:
            warnings.warn("Random object rotation has no effect on history scans since it is assumed to be unlabeled")
    ...
    def random_object_scaling(self, data_dict=None, config=None):
        ...
        # print("ROS gt_box shape", data_dict['gt_boxes'].shape)
        points, gt_boxes = augmentor_utils.scale_pre_object(
            data_dict['gt_boxes'][:, :7], data_dict['points'],
        ...
        if data_dict.get('history_scans', None) is not None:
            warnings.warn("Random object scaling has no effect on history scans since it is assumed to be unlabeled")
        return data_dict
    ...
    def normalize_object_size(self, data_dict=None, config=None):
        ...
        if data_dict.get('history_scans', None) is not None:
            warnings.warn("Normalize object size has no effect on history scans since it is assumed to be unlabeled")
        return data_dict
