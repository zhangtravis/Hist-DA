...
from ithaca365.utils.geometry_utils import transform_matrix
from pyquaternion import Quaternion

# TODO: remove after re-decoing LiDAR into normal frame
# _LiDAR_adjust_qua = Quaternion(axis=(0.0, 1.0, 0.0), degrees=6.25) * Quaternion(axis=(0.0, 0.0, 1.0), radians=np.pi * 3 /4)
# _LiDAR_adjust_mat = _LiDAR_adjust_qua.transformation_matrix
...
def fill_trainval_infos(data_path, nusc, train_scenes, val_scenes, test=False, max_sweeps=10,
                        only_accurate_localization=False):
    train_nusc_infos = []
    val_nusc_infos = []
    progress_bar = tqdm.tqdm(total=len(nusc.sample
                                   if not only_accurate_localization else
                                   nusc.sample_with_accurate_localization),
                             desc='create_info', dynamic_ncols=True)
    ...
    for index, sample in enumerate(nusc.sample
        ...
        ref_cam_front_token = sample['data']['cam0']
        ref_cam_path, _, ref_cam_intrinsic = nusc.get_sample_data(ref_cam_front_token)
        cam_sd_record = nusc.get('sample_data', ref_cam_front_token)
        cam_cs_rec = nusc.get('calibrated_sensor', cam_sd_record['calibrated_sensor_token'])
        imsize = (cam_sd_record['width'], cam_sd_record['height'])
        ...
        info = {
            ...
            # 'lidar_path': Path(ref_lidar_path).__str__(),
            # 'cam_front_path': Path(ref_cam_path).__str__(),
            'cam_intrinsic': ref_cam_intrinsic,
            'token': sample['token'],
            'sweeps': [],
            'ref_from_car': ref_from_car,
            'car_from_global': car_from_global,
            'ref_to_cam': ref_to_cam,
            'imsize': imsize,
            'timestamp': ref_time,
        }
...
def lidar_nusc_box_to_global(nusc, boxes, sample_token):
    ...
    box_list = []
    for box in boxes:
        # Move box back to original LiDAR coord sys
        # box.rotate(_LiDAR_adjust_qua.inverse)
        # Move box to ego vehicle coord system
...
def format_nuscene_results(metrics, class_names, version='default'):
    ...
    for name in class_names:
        ...
        for det_range in metrics['label_aps'][name].keys():
            threshs = ', '.join(list(metrics['label_aps'][name][det_range].keys()))
    ...
    for name in class_names:
        result += f'***{name}\n'
        result += f"match {match_thresh[name]}m:\t" + ', '.join(list(metrics['label_aps'][name].keys())) + '\n'
        metrics_by_ranges = []
        for det_range in metrics['label_aps'][name].keys():
        ...
        result += ", ".join(['%.2f' % (x * 100) for x in metrics_by_ranges]) + "\n"
