...
def parse_config():
    ...
    # parser.add_argument('--pretrained_model_prefix', type=str, default="", help='pretrained_model')
    # parser.add_argument('--pretrained_model_includes', type=str, default=[], nargs='*', help='pretrained_model')
    # parser.add_argument('--pretrained_freeze', action='store_true', default=False, help='whether or not to freeze the backbone')
    parser.add_argument(
    ...
    parser.add_argument(
        "--ckpt_save_interval", type=int, default=10, help="number of training epochs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="empty cuda cache every x iter",
    )
...
def main():
    ...
    if args.pretrained_model is not None:
        no_update_keys = model.load_params_from_file(
            filename=args.pretrained_model, to_cpu=dist_train, logger=logger
        )
    else:
        no_update_keys = []

    optimizer = build_optimizer(model, cfg.OPTIMIZATION, no_update_keys=no_update_keys)
    ...
    # Only evaluate the last 3 epochs
    args.start_epoch = max(args.epochs - 2 * args.ckpt_save_interval, 0)
