...
def update_pred(
    ...
    if args.confidence_score_threshold > 0 and len(preds["name"]) > 0:
        if confidences_per_class is None:
            mask_score = np.array(
                [score > args.confidence_score_threshold for score in preds["score"]]
            )
        else:
            mask_score = np.array(
                [
                    score > confidences_per_class[name]
                    for name, score in zip(
                        preds["name"],
                        preds["score"]
                        if not args.posterior_regularization.by_aum
                        else compute_aum(preds["cls_pred"]),
                    )
                ]
            )
        ...
        mask_score_soft = np.array(
            [score > args.soft_confidence_score_threshold for score in preds["score"]]
        )
    ...
    if getattr(args, "check_consistency", None) is not None:
        ...
        if preds_boxes.shape[0] > 0 and len(previous_det_bbox) > 0:
            ...
            sub_mask = is_consistent_det.sum(axis=0) >= (
                args.check_consistency.ratio * len(previous_det_bbox)
            )
            mask[mask] = sub_mask
...
def process_one_scene(
    idx, args, det_bboxes, previous_det_bboxes=(), confidences_per_class=None
):
    ...
    if args.dataset == "lyft":
        ...
        pp_score_path = osp.join(
            args.data_paths.p2score_path, f"{det_bbox['metadata']['token']}.npy"
        )
...
def process_batch_scene(
    idx_list, args, det_bboxes, previous_det_bboxes=(), confidences_per_class=None
):
    return [
        process_one_scene(
            idx, args, det_bboxes, previous_det_bboxes, confidences_per_class
        )
        for idx in idx_list
    ]
...
def main(args):
    ...
    det_bboxes_new = []
    print("number of detection box scenes:", len(det_bboxes))
    det_bboxes_id = ray.put(det_bboxes)
    previous_results_id = ray.put(previous_results)
    print("... previous box scenes:", len(previous_results))
    ...
    if getattr(args.det_filtering, "posterior_regularization", []):
        ...
        args.det_filtering.confidence_score_threshold = 999.0
        ...
        for frame_detection in det_bboxes:
            if len(frame_detection["name"]) > 0:
                for name, score in zip(
                    ...
                    frame_detection["score"]
                    if not args.det_filtering.posterior_regularization.by_aum
                    else compute_aum(frame_detection["cls_pred"]),
                ):
        ...
        # set threshold
        confidences_per_class = {}  # some values diction
        ...
        for name in confidences:
            ...
            if percentile is not None:
                ...
                num_box_per_class = max(
                    int(len(confidences[name]) * (1 - percentile)),
                    args.det_filtering.posterior_regularization.min_num_boxes,
                )
            else:
                reg_ratio *= (
                    args.det_filtering.posterior_regularization.alpha
                )  # multiply by confidence constant
                num_box_per_class = int(
                    reg_ratio * len(det_bboxes)
                )  # int(len(confidences[name]) * (1 - percentile))
            ...
            print(
                "Posterior regularization values:",
                reg_ratio,
                num_box_per_class,
                confidences_per_class[name],
            )

    else:
        print("No detection filtering... continuing")

    idx_list = np.arange(len(det_bboxes), dtype=int)
    for idx_sublist in np.array_split(idx_list, args.n_processes):
        det_bboxes_new.append(
            process_batch_scene.options(memory=1024 * 1024 * 1024).remote(
                idx_sublist,
                args,
                det_bboxes_id,
                previous_results_id,
