...
def compute_and_save_p2_score_lyft(args):
...
def compute_and_save_p2_score_ithaca365(args):
    ...
    # for sample in tqdm(dataset.sample_with_accurate_localization[args.start_idx:args.end_idx]):
        ...
        if osp.exists(osp.join(args.data_paths.p2score_path, sample_token + '.npy')):
            continue

        persistency_score = dataset.get_point_persistency_score(sd_token, num_histories=5, ranges=(0, 70))
        np.save(osp.join(args.data_paths.p2score_path,
                         sample_token), persistency_score.astype(np.float32))

@hydra.main(config_path="configs/", config_name="p2_score.yaml")
def main(args: DictConfig):
    display_args(args)
    if args.dataset == "lyft":
        compute_and_save_p2_score_lyft(args)
    elif args.dataset == "ithaca365":
        compute_and_save_p2_score_ithaca365(args)
    else:
        raise NotImplementedError()
